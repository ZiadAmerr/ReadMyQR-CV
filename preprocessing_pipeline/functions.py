...
def fix_sin_wave(image):
    ...
    condition = (dc_component > magnitude_spectrum) & (magnitude_spectrum > (second_max * 1.05))
...
def fix_low_contrast(image, hist_values):
    if 0 not in hist_values and 255 not in hist_values:
        ...
        _, image_new = cv2.threshold(equalized_image, keys_average, 255, cv2.THRESH_BINARY)
        return image_new
    else:
        return image
...
def fix_salt_pepper(img):
    ...
    _, denoised_image = cv2.threshold(denoised_image, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
...
def fix_tilt(image):
    ...
    lines = cv2.HoughLines(edges, rho=1, theta=np.pi / 180, threshold=64)  # Adjust threshold for smaller QR codes
...
def rotate_image(image):
    ...
    _, binary = cv2.threshold(np.uint8(temp_img), 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
